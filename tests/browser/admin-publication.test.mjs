...
for (const [label, makeFixture, expectedRule] of [
  ['Landing com prova pendente', landingFixture, 'landing-prova'],
  [
    'Comercial com imagem aprovada',
    commercialPublicationFixture,
    'comercial-v8-hero-imagem',
  ],
])
  await test(
    `${label}: botão publica em desktop/mobile e preserva o gate técnico`,
    ...
    async () => {
      ...
      try {
        ...
        for (const [width, height] of [
          ...
          assert.ok(
            result.warnings.some((finding) => finding.rule === expectedRule),
          );
          ...
          await page.screenshot({
            path: `outputs/publication/browser/${f.tenant.brand.vibe}-${width}.png`,
            fullPage: true,
          });
        }
        assert.equal(
          writes.length,
          8,
          'lock, dois snapshots de página e tenant por publicação',
        );
        ...
        assert.equal(
          writes.length,
          8,
          'a recusa técnica não escreve snapshots',
        );
