...
import { blockMeta, blockSchemas, familyOf, isBlockType } from '@/lib/blocks/registry';
...
export function lintPage(page: Pick<Page, 'blocks' | 'type' | 'title' | 'seo'>): Finding[] {
  const findings: Finding[] = [];
  const blocks = page.blocks ?? [];
  const push = (level: Finding['level'], rule: string, message: string, blockId?: string) =>
  ...
  for (const block of known) counts.set(block.type, (counts.get(block.type) ?? 0) + 1);
  for (const [type, count] of counts) {
    if (count > 1 && blockMeta[type as keyof typeof blockMeta].singleton) {
      push('error', 'singleton', `"${type}" aparece ${count} vezes. Deve aparecer uma só vez.`);
    }
  }

  const heroes = known.filter((b) => familyOf(b.type) === 'hero');
  if (heroes.length > 1) push('error', 'hero-unico', 'Mais de um hero na mesma página.');

  // 3. Diversidade de layout: 8+ seções exigem 4+ famílias.
  const content = known.filter((b) => !['nav', 'footer'].includes(familyOf(b.type) ?? ''));
  ...
  const eyebrows = content.filter((b) => typeof b.props.eyebrow === 'string' && b.props.eyebrow.trim());
  const allowed = Math.max(1, Math.floor(content.length / 3));
  if (eyebrows.length > allowed) {
    push('warn', 'eyebrow-budget', `${eyebrows.length} eyebrows para ${content.length} seções. O limite é ${allowed}.`);
  }
  ...
  for (const hero of heroes) {
    ...
    if (headlineLines(headline) > 2) {
      push('error', 'hero-headline', `Headline ocupa cerca de ${headlineLines(headline)} linhas. O limite é 2.`, hero.id);
    }
    const subtext = propText(hero.props, 'subtext');
    const words = subtext.trim() ? subtext.trim().split(/\s+/).length : 0;
    if (words > 20) {
      push('error', 'hero-subtexto', `Subtexto do hero tem ${words} palavras. O limite é 20.`, hero.id);
    }
  }
  ...
  for (const block of known) {
    for (const text of strings(block)) {
      ...
      if (text.includes('—')) {
        push('error', 'travessao', 'Travessão é proibido no copy. Use ponto ou vírgula.', block.id);
      }
      const filler = FILLER.find((term) => lower.includes(term));
      if (filler) push('error', 'copy-generico', `Expressão genérica: "${filler}".`, block.id);
      const placeholder = PLACEHOLDERS.find((term) => lower.includes(term));
      if (placeholder) push('error', 'placeholder', `Texto de exemplo não substituído: "${placeholder}".`, block.id);
    }
  }

  // 7. Estrutura mínima da página.
  if (page.type !== 'thank_you') {
    if (!known.some((b) => familyOf(b.type) === 'nav')) push('warn', 'sem-nav', 'Página sem barra de navegação.');
    if (!known.some((b) => familyOf(b.type) === 'footer')) push('warn', 'sem-rodape', 'Página sem rodapé.');
  }
  if (page.type !== 'post' && page.type !== 'thank_you' && heroes.length === 0) {
    push('warn', 'sem-hero', 'Página sem hero. Toda página de entrada precisa de uma abertura.');
  }

  // 8. Conversão: toda página precisa de um caminho de ação.
  const hasAction = known.some((b) => ['cta', 'form'].includes(familyOf(b.type) ?? ''));
  if (!hasAction && page.type !== 'thank_you' && page.type !== 'post') {
    push('error', 'sem-conversao', 'Nenhum CTA ou formulário. A página não tem para onde levar o visitante.');
  }

  // 9. SEO.
  const seoTitle = page.seo?.title || page.title;
  if (!seoTitle) push('error', 'seo-titulo', 'Página sem título.');
  else if (seoTitle.length > 60) push('warn', 'seo-titulo', `Título com ${seoTitle.length} caracteres. O ideal é até 60.`);
  const description = page.seo?.description ?? '';
  if (!description) push('warn', 'seo-descricao', 'Página sem meta description.');
  else if (description.length > 160) {
    push('warn', 'seo-descricao', `Meta description com ${description.length} caracteres. O limite é 160.`);
  }

  // 10. Página paga precisa de destino de conversão e não pode ser indexada.
  if (page.type === 'paid_lp' && !page.seo?.noindex) {
    push('warn', 'lp-indexavel', 'Landing page paga sem noindex. Ela pode competir com as páginas orgânicas.');
  }
...
export function formatFindings(findings: Finding[]): string {
  ...
  return findings
    .map((f) => `${f.level === 'error' ? 'ERRO' : 'aviso'} [${f.rule}] ${f.message}`)
    .join('\n');
}
