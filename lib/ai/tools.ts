...
export function buildTools(tenant: Tenant, context: ToolContext = {}) {
  ...
  const tools = {
    ...
    edit_page: tool({
      description:
        'Aplica em uma única gravação todas as edições pedidas na página: replace_text literal; set/unset por caminho; insert_item, move_item e remove_item por caminho e índice; insert/move/remove de bloco; replace_block para trocar a estrutura. Prefira para sites existentes. Exige a revisão do contexto atual/get_page; ambiguidade, conflito ou erro recusa o lote inteiro. Preserva os demais campos e o publicado. Cor local: presentation.background em hex ou transparent; use backgroundEnd com gradient down, diagonal ou right para degradê. Alinhamento: textStyles.align muda somente um campo; presentation.textAlign muda todo o texto; presentation.contentAlign move grupo, botões e listas. Para qualquer ajuste interno sem prop dedicada, use presentation.elements: escolha target section, container, content, heading, body, actions, list, item, media, image, form, action ou field; combine viewport all/mobile/desktop com display, direction, wrap, justify, alignItems, alignSelf, textAlign, columns, gap, dimensões, espaçamentos, ordem, deslocamento, raio, opacidade, cores, borda ou sombra. target item aceita index zero-based. Use uma entrada por alvo/índice/viewport e preserve as demais. Para tipografia exata por campo, textStyles também aceita fontSize, fontWeight, lineHeight, letterSpacing, transform e fontStyle. Não use layout, imagePosition ou center como alternativa para um pedido de text-align à direita. Recusa operações que apaguem texto que o pedido atual não mandou remover. remove apaga a seção inteira e exige autorização desse tamanho. Uma edição visual salva é medida em 1440 e 390 px sem chamada de modelo; o retorno declara falha ou indisponibilidade. O retorno já inclui o pre-flight: não revise ou leia novamente sem necessidade.',
