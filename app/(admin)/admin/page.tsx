  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-6 py-14">
      <header className="mb-12 flex items-end justify-between gap-6">
        <div>
          <h1 className="text-2xl font-semibold tracking-[-0.02em]">Clientes</h1>
          <p className="mt-1 text-sm text-[var(--color-muted)]">
            {tenants.length} {tenants.length === 1 ? 'site' : 'sites'} no painel.
          </p>
        </div>
        <form action={logoutAction}>
          <button className="text-sm text-[var(--color-muted)] hover:text-[var(--color-text)]">Sair</button>
        </form>
      </header>

      <section className="mb-14">
        <h2 className="mb-4 text-sm font-medium uppercase tracking-[0.14em] text-[var(--color-muted)]">Novo cliente</h2>
        <form action={createTenantAction} className="grid gap-3 sm:grid-cols-4">
          <input
            name="name"
          ...
          <input
            ...
            required
            pattern="[a-z0-9-]+"
            className="rounded-md border bg-[var(--color-surface)] px-3.5 py-2.5 text-sm font-mono outline-none focus:border-[var(--color-accent)]"
          />
          <input
            name="whatsapp"
        ...
        <ul className="grid gap-px overflow-hidden rounded-lg border bg-[var(--color-line)] sm:grid-cols-2">
          {tenants.map((tenant) => (
            <li key={tenant.id} className="bg-[var(--color-surface)]">
              <a href={`/admin/${tenant.slug}`} className="flex flex-col gap-3 p-6 hover:bg-[var(--color-surface-2)]">
