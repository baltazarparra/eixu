...
export async function POST(request: Request, { params }: { params: Promise<{ tenant: string }> }) {
  ...
  if (!ALLOWED.has(file.type)) return Response.json({ error: `Tipo não aceito: ${file.type}` }, { status: 415 });
  if (file.size > MAX_BYTES) return Response.json({ error: 'Imagem acima de 8 MB.' }, { status: 413 });

  const kind = form.get('kind') === 'logo' ? 'logo' : 'media';
  const safeName = file.name.toLowerCase().replace(/[^a-z0-9.]+/g, '-').replace(/^-+|-+$/g, '') || 'imagem';
  const blob = await put(`tenants/${tenant.slug}/${kind}/${Date.now()}-${safeName}`, file, {
    access: 'public',
    addRandomSuffix: false,
    contentType: file.type,
  });

  return Response.json({ url: blob.url, name: file.name, type: file.type, size: file.size });
}
