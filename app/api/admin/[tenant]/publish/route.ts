...
export async function POST(request: Request, { params }: { params: Promise<{ tenant: string }> }) {
  ...
  const targets =
    typeof body.page === 'string' ? [await getPage(tenant.id, body.page)].filter(Boolean) : await listPages(tenant.id);
  ...
  for (const page of targets) {
    ...
    if (findings.some((f) => f.level === 'error')) {
      blocked.push({ page: `/${page.slug}`, preflight: formatFindings(findings) });
      continue;
    }
    await db()`
  ...
  if (published.length) await db()`update tenants set status = 'published' where id = ${tenant.id}`;
  return Response.json({ published, blocked, url: `https://${tenant.slug}.eixu.com.br` });
}
