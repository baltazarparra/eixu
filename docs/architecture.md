...
6. `publish_page`, `publish_site` e a API `/api/admin/[tenant]/publish` usam `lib/sites/publish.ts`. O serviço executa `lintPage`, `lintSite` e a trava estrutural antes de copiar os valores validados de `blocks`/`seo` para o snapshot em uma única transação. Só achados de nível erro recusam o lote, e recusam o lote inteiro; avisos do contrato (proporção de imagem, ritmo tonal, silhueta repetida) ficam no painel e no relatório da revisão sem impedir a publicação, o mesmo critério que habilita o botão Publicar. A resposta traz uma entrada por página recusada com todos os motivos, e o painel mostra o caminho e o primeiro motivo. A comparação de silhueta com outros clientes acontece só neste gate, não no estado do painel. Na publicação pontual, páginas fora do lote contam pelo snapshot publicado.
...
A rota do chat tem duração máxima de 800 segundos, com 760 no SDK, e limite de 32 passos na edição livre; as fases usam 12/4/24/32 passos. O último passo da revisão força uma conferência quando ainda houver uma das três leituras disponíveis. Uma conferência completa sem erros após o refinamento encerra o loop. Modelo, raciocínio `high` e saída por tarefa vêm de `lib/ai/models.ts`; `lib/ai/agent.ts` é compartilhado com os runners.

No servidor, a revisão pode continuar por até três rodadas na mesma execução, desde que registre uma leitura ou altere o rascunho. O marcador usa a contagem de leituras e o fingerprint, em vez de interpretar o nome repetido da fase como ausência de avanço. Ao receber timeout do SDK, o runner relê o progresso e decide pela mesma regra. Falha, ausência de avanço ou teto de rodadas encerram com motivo no chat e na linha do tempo. Os limites não provam conclusão; a revisão precisa continuar completa, atual e sem erros. O estado usa `generation_runs.progress` e o payload dos eventos existentes, sem migração.
