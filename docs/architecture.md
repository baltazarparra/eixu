...
2. A criação do tenant exige nome, slug, oferta e ação principal. O primeiro bloco do formulário mostra esses dados e a escolha visual; público, região, evidências, restrições, referências, contatos, logo e cores ficam em seções opcionais. O intake validado permanece em `tenants.brief.intake`. Telefones, endereços e redes sociais vão para `tenants.contacts`; o primeiro WhatsApp é derivado para `tenants.whatsapp`, e a primeira rede suportada alimenta a leitura de perfil. Cada vibe mostra uma amostra comparável e uma paleta sugerida. `paletteSource` só vira `operador` quando uma cor é editada; a sugestão pode ser adaptada pela direção de arte. O logo opcional sobe ao Blob antes do insert e é limpo se o cadastro falhar. O workspace reidrata as 60 mensagens textuais mais recentes do canal. Slug duplicado é recusado sem abrir ou sobrescrever o cliente existente.
3. A interface apresenta três etapas: **Preparar**, **Criar** e **Conferir**. O servidor preserva quatro checkpoints (`briefing`, `cenas`, `composicao`, `revisao`) para retomar com precisão. Preparar lê referências, fixa o guia e persiste o perfil visual v3, o plano editorial e pedidos semânticos para todas as vagas de imagem. Se esses pedidos estão válidos, o runner executa `prepare_site_images` diretamente, sem um turno do coordenador apenas para repetir o plano. Criar monta até 12 páginas com 1–20 blocos; Conferir valida estrutura e pixels. O chat livre ainda oferece logos e ferramentas menores, sempre resolvidas dentro do tenant.
