...
5. `lib/taste/pendencias.ts` converte os achados do pre-flight no que resolve cada um: `alinhar` (bloco, caminho e a frase confirmada que sustenta a alegação), `confirmar` (a frase que falta, se o operador já a escreveu no turno e se cabe no chat ou só em Dados), `imagem` (número, proporção atual e exibida, fotos da biblioteca que cabem, layouts do mesmo bloco e geração por `update_image` com `ratio`) ou `manual`. O plano entra no prompt do turno de edição, ao lado das frases confirmadas, e no retorno de `lint_site`, `lint_page`, `edit_page` e `confirm_evidence`; `lint_page` passou a somar as regras de site da própria página. A regra de prova compara evidência e alegação ignorando acento, caixa e ponto final, pelo mesmo predicado que o plano usa. Publicação continua dependendo do pedido do operador.
