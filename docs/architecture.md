...
- **Revisão visual:** `review_pages` devolve apontamentos estruturais sempre. Com `EIXU_REVIEW_CAPTURE=1` ela também abre o rascunho em Chromium e acrescenta a medição do navegador; as capturas não voltam ao modelo como imagem, porque o base64 no histórico estourou o limite de contexto. A captura busca a URL de preview, que não verifica sessão.
