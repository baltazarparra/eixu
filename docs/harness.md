...
A navegação repete uma vez por viewport antes de desistir, com o critério de espera cedendo de `networkidle2` para `domcontentloaded` e o orçamento de requisições zerado a cada tentativa. O documento de primeiro nível tem prazo próprio, maior que o dos sub-recursos e menor que o do `goto`. Recusa do clip de página inteira cai para o viewport e marca `truncated`. A falha é classificada em `reason` (`rede`, `tempo`, `resposta`, `captura`, `desconhecido`) por `lib/references/failure.ts`, gravada no `excerpt` da evidência e devolvida ao modelo como frase pronta: o agente repassa esse texto e não infere que a origem está fora do ar ou que bloqueia captura, porque não observou o servidor.
...
A causa da parada acompanha cada retomada. `segmentDiagnosis` usa a parte de erro do stream quando ela existe e, quando `finishReason: error` chega sem ela, cai no rastro que o provedor deixou no último passo — `rawFinishReason`, as ferramentas tentadas e `warnings`. O detalhe entra no evento `model.recovering` e na mensagem final do turno, ao lado do `finishReason`. Sem isso, uma assinatura de raciocínio inválida, um corte de contexto e uma recusa do provedor chegam ao operador com o mesmo texto e nenhum deles é investigável depois.

A orientação de retomada depende da causa. `MALFORMED_FUNCTION_CALL` é o Gemini falhando em ler a chamada que ele mesmo gerou, tipicamente por argumento longo e cheio de escape — escrever uma página inteira em uma só chamada de `write_project_file`. Nesse caso `recoveryInstruction` nomeia o problema e manda trocar de tática: um arquivo por chamada, versão inicial menor e complemento por `edit_project_file`. Repetir o empurrão genérico reproduziria a mesma falha até esgotar as tentativas. O prompt dá a mesma orientação antes da primeira falha, porque prevenir custa menos que retomar.
