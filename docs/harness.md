...
Cada viewport tem orçamento próprio (50 s dentro de um teto de 120 s) e uma segunda tentativa de navegação. Um prazo único para tudo derrubava a captura inteira quando o desktop demorava, e uma queda momentânea no documento (`net::ERR_FAILED`) descartava o que já havia sido obtido. Com um viewport entregue, a ferramenta responde `ok` e declara o que faltou em `missingViewports`; o agente relata a limitação em vez de supor o layout. Sem nenhum viewport, continua sendo `unavailable`.
